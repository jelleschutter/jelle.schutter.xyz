  ...
  <div class="work-experiences--wrapper">
      <div :class="'work-experiences--item work-experiences--' + (is_even ? 'left' : 'right')">
          <div class="work-experiences--content">
                <h3 class="work-experiences--company" v-if="has_company_changed">{{ work_experience.company }}</h3>
              <div class="work-experiences--position">
                  <h4>{{ work_experience.job_title }}</h4>
                  <p class="work-experiences--date">{{ formattedStartDate }} &mdash; {{ typeof work_experience.end_date === 'string' ? formattedEndDate : 'Now' }}</p>
                  <p>{{ work_experience.description }}</p>
                  <div class="work-experiences--tick"></div>
              </div>
          </div>
      </div>
      <div :class="'work-experiences--filler work-experiences--' + (is_even ? 'right' : 'left')"></div>
  </div>
</template>
